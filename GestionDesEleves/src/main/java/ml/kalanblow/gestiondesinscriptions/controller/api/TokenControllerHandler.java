...
public class TokenControllerHandler {
    ...
    public ResponseEntity<ErrorResponse> handleRefreshTokenException(TokenException ex, WebRequest request){
        final ErrorResponse errorResponse = ((ErrorResponse) ErrorResponse.builder(ex, HttpStatusCode.valueOf(403), ex.getMessage()));
    ...
    public ResponseEntity<String> handleException(HttpMessageNotReadableException ex) {
        return new ResponseEntity<>("Cannot parse JSON :: accepted roles "+ Arrays.toString(UserRole.values()),HttpStatus.BAD_REQUEST);
    }
